...
TEST_CASE("Reference audio identity copies samples") {
  ...
  ds::reference::copy_samples(std::span<const float>(src), std::span<float>(dst));
...
TEST_CASE("Reference audio gain scales float samples") {
  ...
  ds::reference::gain_samples(std::span<const float>(src), std::span<float>(dst), 2.0);
...
TEST_CASE("Reference audio gain clamps integer samples") {
  ...
  ds::reference::gain_samples(std::span<const short>(src), std::span<short>(dst), 2.0);
